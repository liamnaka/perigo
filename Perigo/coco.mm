...
static NSString* model_file_name = model_uses_memory_mapping ? @"coco_mapped" : @"coco";
...
// Network Nodes
const string input1 = "ToFloat:0";
const string output1 = "detection_boxes:0";
const string output2 = "detection_scores:0";
const string output3 = "detection_classes:0";
...
- (std::vector<tensorflow::Tensor>)run_inference:(tensorflow::Tensor)image_tensor {
    ...
    if (tf_session.get()) {
        ...
        tensorflow::Status run_status = tf_session->Run(
                                                        {{input1, image_feed}}, {output1,output2,output3}, {}, &output);
        if (!run_status.ok()) {
